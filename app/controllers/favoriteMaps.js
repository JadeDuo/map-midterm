...
$(document).ready(() => {
  $.ajax({
    method: 'GET',
    url: '/api/users/fave_list'
  })
    .done((response) => {
      // addMarkersArray() // Take me out if you want
      const $faveList = $('#fave-map-list');
      $faveList.empty();

      for (const map of response.maps) {  // Add list of maps associated with user ID
        $(`<a class="fave-list" href="/api/mapsdata/${map.id}">`).text(map.title).appendTo('#fave-map-list');
      }
    })
  ...
  $(document).on('click', '.fave-list', mapListListener)
  app.onViewChange = function () {
    $(document).off('click', '.fave-list', mapListListener)
